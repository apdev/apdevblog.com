...
package gs {
	...
	public class TweenMax extends TweenLite implements IEventDispatcher {
		...
		private static var _versionCheck:Boolean = (TweenLite.version < 10.09) ? trace("TweenMax error! Please update your TweenLite class or try deleting your ASO files. TweenMax requires a more recent version. Download updates at http://www.TweenMax.com.") : true;
		...
		public function setDestination($property:String, $value:*, $adjustStartValues:Boolean=true):void {
			...
			if (this.initted) {
				...
				for (i = addedTweens.length - 1; i > -1; i--) {
					if (addedTweens[i].name == "_MULTIPLE_") {
						overwriteProps = addedTweens[i].target.overwriteProps;
						for (j = overwriteProps.length - 1; j > -1; j--) {
							v[overwriteProps[j]] = true;
						}
					} else {
						v[addedTweens[i].name] = true;
					}
				}
				killVars(v);
