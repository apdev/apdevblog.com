...
s3cmd sync --acl-public --reduced-redundancy --cf-invalidate --verbose --cf-invalidate-default-index --mime-type='text/html; charset=utf-8' --add-header 'Content-Encoding:gzip' --add-header 'Cache-Control:public,max-age=43200' public/* s3://$1/ --exclude '*.*' --include '*.html'
echo "syncing html (gzipped) complete"

echo "syncing js and css (gzipped)"
s3cmd sync --acl-public --reduced-redundancy --cf-invalidate --verbose --add-header 'Content-Encoding:gzip' --add-header 'Cache-Control:public,max-age=43200' public/* s3://$1/ --exclude '*.*' --include '*.js' --include '*.css'
...
echo "syncing rest"
s3cmd sync --acl-public --reduced-redundancy --cf-invalidate --verbose --add-header 'Cache-Control:public,max-age=43200' public/* s3://$1/ --exclude '*.html' --exclude '*.js' --exclude '*.css' --exclude '*.jpg' --exclude '*.png' --exclude '*.gif'
